...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    router.pathname.search("/product") === -1 &&
  ...
  return (
    <>
      ...
      <Provider store={store}>
        ...
        <Overlay />
        <CartSuccessModal />
        {pathArr.includes("register") ||
          pathArr.includes("login") ||
          pathArr.includes("forgot_password") ||
          (pathArr.includes("coming_soon") !== true && <CommonMobileView />)}
        <ToastContainer />
        <SizeModal />
        <CommonModel />
        <DeleteModal />
